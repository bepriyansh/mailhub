...
const Docs = () => {
  const text = `Go to your Google account at https://myaccount.google.com/
  Go to Security
  ...
  return (
    <div className="flex justify-center items-start w-full h-screen p-2">
      <div className="px-4 py-2 border rounded-lg max-h-[calc(100vh-80px)] overflow-auto">
        ...
        <div className="text-lg py-2">{`Enable 2-Step Verification and generate Application password for your google account, then you can use the generated password to send emails using nodemailer. To do so you need to do the following:`}</div>
        <div className="flex flex-col gap-1 text-md bg-cyan-500/20 rounded-lg p-2">
          {lines.map((line, index) => (
            <div key={index} className="flex gap-2 justify-start items-start">
              <div>{index + 1}. </div>
              <p className="break-words">{convertUrlsToLinks(line)}</p>
            </div>
          ))}
        </div>
      </div>
